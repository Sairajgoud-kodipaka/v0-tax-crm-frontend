...
import { getSessionUser, listTicketsAssignedToEmployee } from '@/lib/data/tickets-queries';
...
export default async function EmployeeMessagesPage() {
  ...
  const myTickets = await listTicketsAssignedToEmployee(session.id);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">
          Open a ticket to view the full message thread and reply. Cases listed here are assigned to you.
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {myTickets.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">No tickets assigned to you yet.</p>
          ) : (
